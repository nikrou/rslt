...
if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && !empty($_GET['term'])) {
...
} elseif (!empty($_POST['save_order']) && !empty($_POST['position']) && !empty($_POST['album_id'])) {
    ...
    try {
        ...
        http::redirect($page_url.'&action=edit&id='.(int) $_POST['album_id']);
    } catch (Exception $e) {
        $core->error->add($e->getMessage());
    }
} elseif (!empty($_POST['remove']) && !empty($_POST['songs']) && !empty($_POST['album_id'])) {
    $album_song_manager = new albumSong($core);
    try {
